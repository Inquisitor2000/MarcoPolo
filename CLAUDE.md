...
## TODO
- **Route crossfade on routing mode switch**: When toggling between Footpath and Main St, the route polyline jumps abruptly. Animate the transition — crossfade old route out / new route in, or interpolate the polyline points. Relevant in `MarcoMap.kt` where `walkRoute` drives the polyline and in both ViewModels' `requestRouteUpdate()` where the new route replaces the old one.
